...
- [ ] T009 [P] Unit tests for `isOrderComplete`, `isOrderFinished`, and `PRE_DESIGN_EDITABLE_STATES` membership in `tests/unit/orders/completeness.test.ts` — cover: all-fields-set (incl. `dimensionUnit`) → complete; one field null (including `dimensionUnit` alone, per analyze-pass finding H1) → incomplete; empty array → complete (vacuous); all-terminal → finished; one non-terminal → not finished; `NEW`/`ASSIGNED` in the set, every other `WorkItemState` not
...
- [ ] T023a [US3] Implement `changeOrderPriority(actor, orderId, priority)` in `src/server/orders/cancelOrder.ts` per contracts/order-entry.md: `authorize(actor, "order.edit")`; no-op (no write, no audit) if the new value equals the current one; otherwise `tx.order.update` plus `audit.record("order.priority_changed", { before, after })` — implements FR-007's audited priority change and US3 Acceptance Scenario 3 (analyze-pass finding C1)
- [ ] T023b [US3] Export `changeOrderPriority` from `src/server/orders/index.ts`
- [ ] T023c [US3] Add a priority control (NORMAL/URGENT toggle) to each row of `src/app/(shell)/reception/page.tsx`, wired to `changeOrderPriority`, with `revalidatePath("/reception")` after the change so the row re-sorts to the front on the next render (depends on T023, T023a)
