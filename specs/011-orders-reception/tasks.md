...
- [ ] T002 **ACTION REQUIRED — blocked, do not run unattended.** Run `pnpm exec prisma db push` then `pnpm exec prisma db seed` to apply T001's schema and re-seed, per research.md §1's migration note (existing seeded `Order.number` values are pre-production and get truncated by re-seeding, not hand-migrated). **Blocker**: the shared dev Supabase DB has a live-only schema drift (`Customer.isArchived`/`normalizedName`, `CustomerPhone` table) that exists in no git branch — confirm with Fady this drift is safe to reconcile (or capture it into 010's migration first) before pushing, since `db push` diffs against live state and can silently drop those columns/tables. Whoever picks this up: (1) confirm with Fady, (2) run the two commands above, (3) re-run `pnpm exec vitest run tests/unit/workflow-edges.test.ts` to confirm `Order.dueDate` resolves, (4) check off T052 (manual quickstart QA) once seeded data is available.
- [x] T003 [P] Add a `no-restricted-imports` rule for `src/server/orders/**` to `eslint.config.js`, identical in shape to the existing `src/server/core/**`/`src/server/auth/**` rules (research.md §6): only `~/server/orders` (the barrel) is importable from outside the module; `tests/**` exempted
- [x] T004 [P] Create the barrel `src/server/orders/index.ts` with no exports yet (placeholder `export {}` — populated incrementally as each story's functions land)
...
- [x] T005 [P] Implement `DomainOrderError` in `src/server/orders/errors.ts` — a local error class carrying `code: "ORDER_FINISHED" | "PAST_EDIT_WINDOW" | "TERMINAL_WORK_ITEM" | "DUPLICATE_NAME"` and a human-readable message (plan.md §5.3)
- [x] T006 [P] Implement `isOrderComplete()` in `src/server/orders/completeness.ts` exactly per data-model.md: pure function taking `{ workItems: ReadonlyArray<{ productTypeId, quantity, widthValue, heightValue, dimensionUnit, departmentId }> }`, returns `true` only when every Work Item has all six fields non-null (vacuously `true` on an empty array) — `dimensionUnit` is required per spec.md's Clarifications answer ("width × height + unit")
- [x] T007 [P] Implement `isOrderFinished()` in `src/server/orders/completeness.ts` exactly per data-model.md: pure function taking `ReadonlyArray<{ state: WorkItemState }>`, returns `true` only when every item's state is `"DELIVERED" | "COMPLETED" | "CANCELLED"`
- [x] T008 [P] Add `PRE_DESIGN_EDITABLE_STATES: ReadonlySet<WorkItemState> = new Set(["NEW", "ASSIGNED"])` to `src/server/orders/completeness.ts` (data-model.md)
- [x] T009 [P] Unit tests for `isOrderComplete`, `isOrderFinished`, and `PRE_DESIGN_EDITABLE_STATES` membership in `tests/unit/orders/completeness.test.ts` — cover: all-fields-set (incl. `dimensionUnit`) → complete; one field null (including `dimensionUnit` alone, per analyze-pass finding H1) → incomplete; empty array → complete (vacuous); all-terminal → finished; one non-terminal → not finished; `NEW`/`ASSIGNED` in the set, every other `WorkItemState` not
- [x] T010 Add `seedProductTypes()` to `prisma/seed.ts`, called from the existing seed `main()` and upserted by `name` like `seedDepartments()` already is: Roll-up Banner, Business Cards, Flyer/Poster, Vinyl Sticker, Outdoor Sign, Laser-cut Sign, each mapped to one of the five existing seeded departments (Digital, Banner, Outdoor, Laser, External; Business Cards/Flyer default to Digital), `defaultRequiresDesign: true` / `defaultRequiresReview: true` for all except Business Cards which sets `defaultRequiresReview: false` (data-model.md Seed data addition)
...
- [x] T020 [P] [US3] Integration test in `tests/integration/orders/queue.test.ts`: seed orders with mixed priorities/creation times/completeness; asserts `listReceptionQueue` returns urgent orders first, then the rest oldest-first (FR-007a, FR-008); asserts an order with any Work Item missing a completeness field per `isOrderComplete()` is flagged incomplete; asserts the queue still returns correctly when no delayed-Work-Item signal is supplied (FR-008a — 053 optional)
- [x] T020a [P] [US3] Integration test in `tests/integration/orders/changeOrderPriority.test.ts`: changing a NORMAL order to URGENT updates `Order.priority`, records one `audit.record("order.priority_changed")` entry with before/after values, and moves that order ahead of other NORMAL orders in a subsequent `listReceptionQueue` call; setting the same priority again is a no-op (no new audit entry) (FR-007)

### Implementation for User Story 3

- [x] T021 [US3] Implement `listReceptionQueue(actor)` in `src/server/orders/search.ts`: requires only an authenticated `actor` — no specific `Permission` gate (contracts/order-entry.md's `searchOrders` rationale: FR-010/US4 frame order lookup as broader than Reception's own capability set, and 001 froze the `Permission` vocabulary with no `order.view` key); query orders with their Work Items' completeness-relevant fields and states; sort `priority === "URGENT"` first, then `createdAt` ascending within each priority bucket (FR-008); map each row through `isOrderComplete()` for its incomplete flag and through `deriveOrderStatus` (from `~/server/core`) for its derived status; accept an optional `getDelayedWorkItems` (053) result to merge a per-row `delayed` flag when present, defaulting to `false`/absent when 053 hasn't shipped (FR-008a) (depends on T006)
- [x] T022 [US3] Export `listReceptionQueue` from `src/server/orders/index.ts`
- [x] T023 [US3] Build `src/app/(shell)/reception/page.tsx` — Server Component rendering `listReceptionQueue`'s results: urgent badge, incomplete badge with the list of missing fields, derived status badge, links to each order's detail page and to Quick Create / full order form (depends on T021)
- [x] T023a [US3] Implement `changeOrderPriority(actor, orderId, priority)` in `src/server/orders/cancelOrder.ts` per contracts/order-entry.md: `authorize(actor, "order.edit")`; no-op (no write, no audit) if the new value equals the current one; otherwise `tx.order.update` plus `audit.record("order.priority_changed", { before, after })` — implements FR-007's audited priority change and US3 Acceptance Scenario 3 (analyze-pass finding C1)
- [x] T023b [US3] Export `changeOrderPriority` from `src/server/orders/index.ts`
- [x] T023c [US3] Add a priority control (NORMAL/URGENT toggle) to each row of `src/app/(shell)/reception/page.tsx`, wired to `changeOrderPriority`, with `revalidatePath("/reception")` after the change so the row re-sorts to the front on the next render (depends on T023, T023a)
