...
- **FR-021**: The system MUST provide a payments area on the order detail page (filling the placeholder 011 left), the customer-profile Payments & balance tab (010's slot), a record-payment dialog, an expenses list with form, a direct-cost entry form, the profitability view, and the daily cash summary — all Arabic-first and task-oriented: recording a payment takes at most 3 interactions after the dialog opens (amount → method/source preselected → confirm).
...
- **Receipt numbering**: Sequential receipt numbers are per-shop and monotonic; gaps may occur only from rolled-back or unused sequence allocations (database SEQUENCE semantics). Strict gapless fiscal numbering is a tax-invoicing concern, out of scope.
- **V1 config editing**: `FinanceConfig` is seeded from `config/052-finance.yaml` and edited through the validated `admin.config` write helper; no visual config-admin screen is required in V1 — FR-003/FR-014 mandate configurable data, not a specific editor (050's FileConfig Admin UI remains the pattern if one is added later).
...
- Permission vocabulary is frozen in 001 (`payment.record`, `payment.void`, `expense.record`, `finance.view`); any addition must be coordinated through 001, not invented here. The expense-approval transition, the credit flag/limit write (FR-010, FR-013), and `FinanceConfig` writes are performed by Admin/Owner through the existing `admin.config` permission (credit writes additionally require a reason as a 052-local policy validated before `audit.record`); no new permission key is introduced by 052.
