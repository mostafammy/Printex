...
- **FR-006**: The system MUST set the processed marker and its status on the outbox row, and MUST make `deliveredAt` and `deliveryStatus` readable to an Admin for audit and reconstruction. No feature other than 053 writes them.
...
- **Notification Type (catalog entry)**: The shared definition of one event type — stable name, aliases, Arabic title and body template, default recipient specification, deep-link target, owning feature, and whether it is delivered directly or derived from a state change.
...
- **SC-014**: A dealer of 200 notifications across 5 phases for one Work Item is handled without a scheduler failure or an unbounded alert burst — the alert count equals the number of breached Work Items, not the number of ticks.
- **SC-015**: 0% of notifications contain a hard-coded employee name or role name in the delivery rules; every recipient decision traces to configuration or to the catalog (constitution VI).
