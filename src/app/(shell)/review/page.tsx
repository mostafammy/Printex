...
export default async function ReviewQueuePage() {
  ...
  const rows = await getReviewQueue(actor);

  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h1 className="text-xl font-semibold">{S.reviewQueuePageTitle}</h1>
      </div>

      {rows.length === 0 ? (
        <p className="text-muted-foreground">{S.reviewQueueEmpty}</p>
      ) : (
        <div className="overflow-x-auto rounded-lg border border-border">
          <table className="w-full text-sm">
            <thead className="bg-muted text-muted-foreground">
              <tr>
                <th className="px-4 py-3 text-start font-medium">{S.reviewQueueTableHeaderCustomer}</th>
                <th className="px-4 py-3 text-start font-medium">{S.reviewQueueTableHeaderProduct}</th>
                <th className="px-4 py-3 text-start font-medium">{S.reviewQueueTableHeaderEnteredQueue}</th>
                <th className="px-4 py-3 text-start font-medium">{S.reviewQueueTableHeaderRework}</th>
                <th className="px-4 py-3 text-start font-medium">{S.reviewQueueTableHeaderActions}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {rows.map((row) => (
                <tr key={row.workItemId} className="bg-card hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3">
                    <Link
                      href={`/review/${row.workItemId}`}
                      className="rounded-md bg-primary px-3 py-1.5 text-xs font-medium text-primary-foreground hover:opacity-90"
                    >
                      {S.reviewQueueOpenButton}
                    </Link>
                  </td>
                </tr>
              ))}
