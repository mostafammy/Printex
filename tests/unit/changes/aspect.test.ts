...
function createP2002Error(target: readonly string[]) {
  const err = new Error("Unique constraint failed");
  Object.assign(err, {
    code: "P2002",
    meta: { target, modelName: "TestModel" },
  });
  return err;
}

describe("016 changes aspect binding mapUniqueViolation (B4)", () => {
  it("matches STALE_SPEC_VERSION when target includes both workItemId and version", async () => {
    const cmd = defineCommand({
      action: "test.stale_spec",
      input: z.object({}),
      permission: "order.edit",
      run: async () => {
        throw createP2002Error(["workItemId", "version"]);
      },
    });
  ...
  it("does NOT match STALE_SPEC_VERSION for DesignVersion-style target ['designId', 'version']", async () => {
    const cmd = defineCommand({
      action: "test.design_version",
      input: z.object({}),
      permission: "order.edit",
      run: async () => {
        throw createP2002Error(["designId", "version"]);
      },
    });
    ...
    expect(res).toEqual({
      ok: false,
      error: { code: "CONFLICT", entity: "TestModel", id: "" },
    });
  });
  ...
  it("does NOT match STALE_SPEC_VERSION for DesignVersion index name target", async () => {
    const cmd = defineCommand({
      ...
      permission: "order.edit",
      run: async () => {
        throw createP2002Error(["DesignVersion_designId_version_key"]);
      },
    });
  ...
  it("prioritizes STALE_SPEC_VERSION over CHANGE_REQUEST_PENDING when target has workItemId and version", async () => {
    const cmd = defineCommand({
  ...
  it("matches CHANGE_REQUEST_PENDING when target only includes workItemId", async () => {
    const cmd = defineCommand({
